...
class CompanyContactCrudController extends BaseCrudController
{
    ...
    {
        if ($entityInstance->getSex() === 'F' && $entityInstance->getLang() == 'fr') {
            $entityInstance->setGreeting('Chère Madame');
        }

        if ($entityInstance->getSex() === 'F' && $entityInstance->getLang() == 'nl') {
            $entityInstance->setGreeting('Geachte Mevrouw');
        }

        if ($entityInstance->getSex() === 'M' && $entityInstance->getLang() == 'fr') {
            $entityInstance->setGreeting('Cher Monsieur');
        }

        if ($entityInstance->getSex() === 'M' && $entityInstance->getLang() == 'nl') {
    ...
    {
        if ($entityInstance->getSex() === 'F' && $entityInstance->getLang() == 'fr' && empty($entityInstance->getGreeting())) {
            $entityInstance->setGreeting('Chère Madame');
        }

        if ($entityInstance->getSex() === 'F' && $entityInstance->getLang() == 'nl' && empty($entityInstance->getGreeting())) {
            $entityInstance->setGreeting('Geachte Mevrouw');
        }

        if ($entityInstance->getSex() === 'M' && $entityInstance->getLang() == 'fr' && empty($entityInstance->getGreeting())) {
            $entityInstance->setGreeting('Cher Monsieur');
        }

        if ($entityInstance->getSex() === 'M' && $entityInstance->getLang() == 'nl' && empty($entityInstance->getGreeting())) {
            $entityInstance->setGreeting('Geachte Heer');
        }
